...
- **格闘場の再現**: 全 38 試合カードのデータ、主人公レベル帯によるマッチメイク、オッズの乱数補正、賭け金（主人公 Lv × 10G）、払い戻し。全 38 試合がプレイ可能（試合 38「あやしいかげ ×3」は戦闘開始時に正体を抽選し、撃破されると表示名が正体に変わる）。
...
- **保存容量**: localStorage は概ね 5M 文字です。Battle Log（1 試合 約 15KB）は遊んでいる冒険の書の直近 30 試合だけ保持し、他の冊は要約（約 0.8KB）だけ残します。ログは battleSeed から再生成できます。
- **Analysis は Web Worker で実行**します（`src/ui/logic/analysis.worker.ts`）。Worker もメインスレッドと同じ `createDefaultArenaGame()` と `runAnalysis` を使うので、同じ seed ならメインスレッドで回したときと同じ表になります（テストで確認）。
...
- あやしいかげの正体は隊列先頭のレベルで決まるが、隊列を持たないので主人公レベルを先頭のレベルとして使う（Approximation）。能力値・行動はすべて正体から取る（fidelity-review.md U-10）

## Known Differences

- **乱数列は実機と一致しません。** 分布が等価な乱数で、呼び出し口と消費順だけを ROM に合わせています。
- あやしいかげの表示名は撃破時だけ正体に差し替えます（$02BAE8）。生き残った選手は決着時も「あやしいかげ」のまま表示します。差し替えたことはログに注記（実機に無い文）で示します。
- 予想屋は未実装です。
- 引き分け時の払い戻しは、賭けた選手が生き残った引き分けだけ返金します（攻略サイトの記述による）。
