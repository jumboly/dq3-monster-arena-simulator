 ...
 *
 * 画面からは analysisClient 経由で Web Worker 内で呼ばれ、Worker が使えないときだけメインスレッドで動く。
 * どちらでも同じこの関数を通すので、同じ seed なら実行場所によらず同じ表になる。
 * 時間予算（既定 12ms）で区切って次のチャンクを setTimeout に回すのは、メインスレッドでは描画を、
 * Worker では中止メッセージの受信を、チャンクの合間に通すため。
